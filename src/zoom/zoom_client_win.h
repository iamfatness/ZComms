...
class ZoomClientWin : public ZoomClient,
 ...
 public:
  ...
  // Windows-only extra: this is the `--anon` guest path and is not on the
  // seam.
  ...
  bool Join(uint64_t meeting_number, const std::string& password,
            ...
            const std::string& zak = std::string()) override;
  ...
  PasscodeState passcode_state() const override {
    return static_cast<PasscodeState>(passcode_state_.load());
  }
  bool SubmitPasscode(const std::string& passcode) override;

  // setExternalAudioSource. This one call is the entire TX path (plan §2).
  std::unique_ptr<VirtualMic> InstallVirtualMic(std::string* error) override;

  bool JoinVoip(std::string* error) override;
  bool LeaveVoip(std::string* error) override;
  ...
  bool UnmuteSelf(std::string* error) override;
  ...
  void LogSelfAudioState(const char* tag) override;

  // The talkback controller for this meeting, already wrapped.
  ...
  MeetingState state() const override;
  int last_fail_code() const override { return last_fail_code_.load(); }
  std::string FailReason(int code) const override;
