...
// MeetingStatus is Zoom's enum, with many values ZComms does not model (e.g.
// DISCONNECTING, LOCKED, the webinar promote/demote pair); they all mean "not
// one of the states we act on" and collapse to Idle. Names verified against
// the existing MeetingStatusName below, which has compiled against the real
// header in every prior CI run.
...
std::string ZoomClientWin::FailReason(int code) const {
  switch (code) {
    ...
    case ZoomClientWin::kFailAccountBusyElsewhere:
...
ZoomClientWin::~ZoomClientWin() { Cleanup(); }

void ZoomClientWin::Pump(int ms) {
...
bool ZoomClientWin::Init(std::string* error) {
  InitParam param;
...
bool ZoomClientWin::AuthenticateWithJwt(const std::string& jwt, int timeout_ms,
                                        std::string* error) {
...
bool ZoomClientWin::Authenticate(const std::string& public_app_key,
...
bool ZoomClientWin::Join(uint64_t meeting_number, const std::string& password,
  ...
  while (NowNs() < deadline) {
    ...
    if (s == MEETING_STATUS_FAILED) {
      *error = "join failed: " + FailReason(last_fail_code_.load());
      return false;
    }
    if (s == MEETING_STATUS_ENDED) {
      const int code = last_fail_code_.load();
      *error = code != 0 ? "join failed: " + FailReason(code)
...
bool ZoomClientWin::JoinVoip(std::string* error) {
  if (meeting_ == nullptr) {
...
bool ZoomClientWin::LeaveVoip(std::string* error) {
  if (meeting_ == nullptr) {
    *error = "no meeting service";
    return false;
  }
...
bool ZoomClientWin::UnmuteSelf(std::string* error) {
  if (meeting_ == nullptr) {
    *error = "no meeting service";
    return false;
  }
...
void ZoomClientWin::onAuthenticationReturn(AuthResult ret) {
  auth_result_.store(static_cast<int>(ret));
  auth_returned_.store(true);
  std::printf("[sdk] onAuthenticationReturn: %s\n", AuthResultName(ret));
}

void ZoomClientWin::onLoginReturnWithReason(LOGINSTATUS, IAccountInfo*,
                                            LoginFailReason) {}
void ZoomClientWin::onLogout() {}
void ZoomClientWin::onZoomIdentityExpired() {
  std::printf("[sdk] zoom identity expired\n");
}
void ZoomClientWin::onZoomAuthIdentityExpired() {
  std::printf("[sdk] zoom auth identity expires soon\n");
}
void ZoomClientWin::onNotificationServiceStatus(SDKNotificationServiceStatus,
                                                SDKNotificationServiceError) {}

// --- IMeetingServiceEvent ---------------------------------------------------

void ZoomClientWin::onMeetingStatusChanged(MeetingStatus status, int result) {
  ...
  std::printf("[sdk] meeting status: %s%s\n", MeetingStatusName(status),
              (status == MEETING_STATUS_FAILED)
                  ? (" (" + FailReason(result) + ")").c_str()
                  : "");
}

void ZoomClientWin::onInputMeetingPasswordAndScreenNameNotification(
...
// opaque ENDED.
void ZoomClientWin::onJoinMeetingNeedUserInfo(
...
void ZoomClientWin::onWebinarNeedRegisterNotification(
    IWebinarNeedRegisterHandler*) {
  std::printf("[sdk] this webinar requires registration -- cannot join\n");
}
void ZoomClientWin::onEndOtherMeetingToJoinMeetingNotification(
...
void ZoomClientWin::onWebinarNeedInputScreenName(IWebinarInputScreenNameHandler*) {
  std::printf("[sdk] webinar wants a screen name prompt\n");
}

bool ZoomClientWin::SubmitPasscode(const std::string& passcode) {
...
void ZoomClientWin::onMeetingStatisticsWarningNotification(StatisticsWarningType) {}
void ZoomClientWin::onMeetingParameterNotification(const MeetingParameter*) {}
void ZoomClientWin::onSuspendParticipantsActivities() {}
void ZoomClientWin::onAICompanionActiveChangeNotice(bool) {}
void ZoomClientWin::onMeetingTopicChanged(const zchar_t*) {}
void ZoomClientWin::onMeetingFullToWatchLiveStream(const zchar_t*) {}
void ZoomClientWin::onUserNetworkStatusChanged(MeetingComponentType,
                                               ConnectionQuality, unsigned int,
                                               bool) {}
void ZoomClientWin::onAppSignalPanelUpdated(IMeetingAppSignalHandler*) {}
...
bool ZoomClientWin::AdmitAllWaiting() {
  if (meeting_ == nullptr) return false;
...
IMeetingBOController* ZoomClientWin::GetBOController() {
  return meeting_ != nullptr ? meeting_->GetMeetingBOController() : nullptr;
}

IMeetingChatController* ZoomClientWin::GetChatController() {
  return meeting_ != nullptr ? meeting_->GetMeetingChatController() : nullptr;
}

std::vector<unsigned int> ZoomClientWin::GetOtherParticipants() {
...
void ZoomClientWin::LogSelfAudioState(const char* tag) {
...
void ZoomClientWin::onUserAudioStatusChange(IList<IUserAudioStatus*>* list,
                                            const zchar_t*) {
...
void ZoomClientWin::onUserActiveAudioChange(IList<unsigned int>*) {}

void ZoomClientWin::onHostRequestStartAudio(IRequestStartAudioHandler* handler) {
...
void ZoomClientWin::onJoin3rdPartyTelephonyAudio(const zchar_t*) {}

void ZoomClientWin::onMuteOnEntryStatusChange(bool enabled) {
  std::printf("[audio] mute-on-entry is %s\n", enabled ? "ON" : "off");
}

// --- Factories: no SDK pointer crosses the ZoomClient seam ------------------
