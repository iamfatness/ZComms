...
**Deliberately not touched:** `src/zoom/duck_plan.*`, `reach.*`, `room_plan.*`, `signal_*.*`, and everything in `src/audio/` *except* `mic_source.cpp`. They are already portable. **The port is the boundary, not the product.**

> **Amendment, 2026-09-05, after execution.** The line above originally also named `roster.*`, `breakout.*`, `chat_signals.*`, and `talkback_source.*` as already-portable and untouched-because-unnecessary. That was wrong in the same way the design spec's §2 was wrong (see the amendment there, dated the same day): `CMakeLists.txt:171-179` puts all four in the Windows-only source list. `roster.cpp`, `breakout.cpp`, and `chat_signals.cpp` are genuinely SDK-bound (`using namespace ZOOM_SDK_NAMESPACE`, built on the SDK's C++ interface types) and need their own seam-and-port work, same shape as `TalkbackSdk`. `talkback_source.*` is Windows-only for the same reason. `src/audio/mic_source.cpp` (not `jwt.cpp`, which has no SDK types and is a much smaller BCrypt→CommonCrypto swap — see the design spec amendment) is also Windows-only, implementing `IZoomSDKVirtualAudioMicEvent`. None of this was in scope for this plan — it only extracts the `TalkbackSdk` seam — but the "deliberately not touched" framing implied these were already done, and they are not.
