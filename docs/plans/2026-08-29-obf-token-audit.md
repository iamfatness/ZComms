> **SATISFIED BY ZAK (owner ruling, 2026-08-30):** the requirement accepts an OBF token, a ZAK, OR an on-behalf token -- ZComms sends the operator's ZAK on every signed-in join, and cross-account joins are live-proven (meeting 96553474365 joined and operated 2026-08-29/30). No OBF plumbing needed; kept for reference if a ZAK-less join mode ever appears.
...
1. **Where `JoinParam` is populated:** `src/zoom/zoom_client.cpp`, `ZoomClient::Join()` (~line 258). It sets `meetingNumber`, `userName`, `psw`, and â€” only when the caller passes one â€” `userZAK`. It **never** sets `app_privilege_token`, `onBehalfToken`, or `join_token`. The fields exist in the vendored header; ZComms simply doesn't plumb them.
2. **Auth flows in play:** (a) signed-in: PKCE â†’ broker â†’ `AuthenticateWithJwt` + ZAK from `ZoomOAuth::EnsureJoinCredentials` on `JoinParam.userZAK` â€” this satisfies the mandate for meetings the *signed-in user* can join, because a ZAK is one of the accepted credentials; (b) `--anon`: bare public-app-key guest join with no token at all â€” this is the non-compliant path for cross-account meetings and already fails them with `MEETING_FAIL_APP_CAN_NOT_ANONYMOUS_JOIN_MEETING` (504), decoded at `zoom_client.cpp` (`MeetingFailReason`).
3. **The gap:** a signed-in operator joining a meeting hosted by a *different* account (the client's production meeting â€” ZComms's core use case) presents a ZAK identifying the operator, which authenticates the user but does not by itself carry the app's cross-account privilege; the OBF `app_privilege_token` is the credential designed for that case. Today the outcome depends on Zoom-side enforcement state; after full enforcement the join can fail even signed-in.
4. **Not found in the vendored SDK:** any API to *mint* an OBF/app-privilege token (correct â€” it comes from Zoom's REST side, tied to the Marketplace app identity, not from the Meeting SDK), and any distinct MeetingFailCode for "token required/expired" beyond 504 (so error surfacing keys on 504 plus join context). Minting therefore lands on the CoreVideo broker; the exact Zoom REST endpoint and required Marketplace scopes are a broker-repo task and are treated here as an interface contract (`/oauth/obf-token`), not as settled fact.
...
2. A pure `join_gate` decides, from what the session has (signed-in? ZAK? OBF token? anon flag?), which tokens the join carries and whether the join should even be attempted â€” with exact operator text for each refusal.
...
4. A 504 join failure reports differently depending on what was sent: anon â†’ the existing "start the meeting from the account that installed ZComms" text; signed-in without OBF â†’ "cross-account meeting needs the app privilege token -- broker OBF support required (see docs/plans/2026-08-29-obf-token-audit.md)"; signed-in with OBF â†’ "Zoom refused the app privilege token -- token may be stale, retry sign-in".
...
- `JoinParam4WithoutLogin` string fields are `const zchar_t*` borrowing from caller-owned storage: every wide string passed must outlive the `meeting_->Join(jp)` call â€” keep the existing local-`std::wstring` pattern of `Join()`.
- Secrets discipline: OBF tokens are short-lived join credentials â€” hold them in memory only, never in the DPAPI store, never in logs (log presence, length class, and expiry only).
- The FAILED code is latched and reported from the ENDED branch (existing trap â€” do not re-break it).
- Update `CLAUDE.md` in the same change as any substantive work.

## Tasks

### Task 1: `join_gate` â€” token decision + refusal text (pure)
...
- [ ] Implement `DecideJoin` / `JoinFailText` in `src/zoom/join_gate.cpp` to the exact truth table the test pins: signed-in â†’ attempt with whatever tokens exist, warn `"no OBF token -- cross-account meetings may refuse this join"` when `obf_token` is empty; not signed-in and not anon â†’ refuse `"sign in first (owner rule: no anonymous joins)"`; anon â†’ attempt bare. `JoinFailText` returns `""` for every code except 504 (the caller falls through to the existing `MeetingFailReason`), and the three 504 variants from the test.
...
**Interfaces**
- Produces (replacing the trailing `const std::string& zak` parameter â€” update the two existing call sites in `src/app/main.cpp` in the same change):
  ```cpp
  struct JoinTokens {
  ...
  ```
- Consumes: `JoinParam4WithoutLogin` fields verified present in `meeting_service_interface.h` (lines 254â€“288 of the vendored header).
...
- [ ] Change the signature in `zoom_client.h`; in `zoom_client.cpp` extend the population block (keeping every wide string a named local so it outlives `meeting_->Join(jp)`):
  ...
  ```
  (presence-only logging â€” Global Constraints forbid token values in logs).
- [ ] Update the call sites in `src/app/main.cpp` (the signed-in join passes `{zak, "", "", ""}` for now; `--anon` passes the default).
- [ ] Build the app target: `cmake --build build --config Release --target zcomms` â€” compile-clean gate; run `build\Release\zcomms_audio_tests.exe` to confirm nothing else moved.
...
- Produces on `ZoomOAuth`:
  ...
  bool EnsureObfToken(uint64_t meeting_number, std::string* obf,
                      std::string* error);
  ```
- Consumes: the broker contract `GET https://corevideo.iamfatness.us/oauth/obf-token?meeting=<n>` with `Authorization: Bearer <access token>`, response `{"obf_token":"...","expires_in":<sec>}` â€” **deploying this endpoint is CoreVideo-repo work** (the Worker holds the Marketplace credentials and calls Zoom's REST side; the exact Zoom endpoint/scopes get confirmed there against current Zoom docs). Until it deploys, the 404 degrade path keeps ZComms shippable.

**Steps**

- [ ] Implement `EnsureObfToken` in `zoom_oauth.cpp` using the same WinHTTP request helper `EnsureJoinCredentials` uses (refresh-if-needed under `m_`, one GET, parse the two fields with the file's existing JSON field scanner); map HTTP 404 â†’ `return true` with `obf->clear()` plus one `[oauth] broker has no OBF endpoint yet -- joining without app privilege token` printf; map 401 â†’ clear tokens and ask for re-sign-in (the established `invalid_grant` rule).
...
- [ ] `JoinFailText` returns `""` for non-504 so existing decodes keep working â€” pinned by test.
